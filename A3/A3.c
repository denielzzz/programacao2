...
void execute_event(space *board, shot_sentinel *list)
{
	...
	for (int i = 1; i <= board->max_y; i++)
		...
		{
			tiro = 0;
			if (board->map[i][j].entity && board->map[i][j].type == ENEMY && board->map[i+1][j].entity == NULL)
			{
				for(shot *k = (shot*) list->first; k; k = (shot*) k->next)
					if (k->position_x == j)
						tiro = 1;
				if(!tiro)
					straight_shot(board, list, (enemy*) board->map[i][j].entity);
			}
		}
}
